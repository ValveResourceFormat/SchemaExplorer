...
import { formatHexOffset } from "./format";
...
function* declarationLines(d: Declaration): Generator<string> {
  if (d.kind === "enum") {
    ...
    for (const f of d.fields) {
      yield `    ${formatFieldType(f.type)} ${f.name}; // ${formatHexOffset(f.offset)}`;
    }
  }
}
...
/** Returns the JSON for a `discord:component-embed` script, fitting as many members as the size limit allows. */
